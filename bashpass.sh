...
declare -a desc=( "exit this menu" "gathter details to generate a new password" "search records by domain" "print all records" "start a sqlite session against your db" "prompt for a csv file to import (eg: test.csv)" "print this message" )
...
function writerecord {
  ...
  ${cmd} "select * from ${act} where dm like '${dm}';"
...
while :; do
  ...
  case "${ui}" in
    ...
    2) read -p "Enter domain to look for: " dm;${cmd} "select rowid as id,* from ${act} where dm like '%${dm}%';"; unset dm;;
    3) ${cmd} "select rowid as id, * from ${act};";;
    4) ${cmd};;
