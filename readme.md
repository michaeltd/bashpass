...
### NOTES
 ...
 2. Consider launching [bashpass.sh](bashpass.sh) with launch from terminal option enabled (where available) and through [bashpass-launcher.sh](bashpass-launcher.sh) if not (DE hotkey for example).
    ...
    For SQLite session availability mainly (as lack there off would mean you'll be left without application control if you'll launch an sql session with the initiating shell unavailable) but also for troubleshooting reason.
